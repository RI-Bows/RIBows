...
import { TrendingRio, getRios } from '@/lib/dbActions';
import { Rio } from '@prisma/client';
// import { IRIO } from '@/lib/validationSchemas';
// import { TrendingRio } from '@/lib/dbActions';

export default function SearchClient({ initialRios }: { initialRios: Rio [] }) {
  const rioList = initialRios;

  const [searchQuery, setSearchQuery] = useState('');
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [rios, setRios] = useState(rioList); // change fallbackTrendingRios to database ones
  const [filteredItems, setFilteredItems] = useState(rioList);
  const [sortBy, setSortBy] = useState('trending');
  const [selectedType, setSelectedType] = useState('all');

  // sorting function
  const sortItems = (items: Rio [], sortType: string) => {
    const sorted = [...items];
  ...
  }, [searchQuery, rios, sortBy, selectedType, filteredItems]);
